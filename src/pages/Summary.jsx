      ...
      <div className="py-4 text-[#1A1A1A] relative flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 font-medium mt-1">
            Global Relationship Director · Maybank Tower HQ · <span className="text-zinc-500 font-semibold">{todayDateStr}</span>
          </p>
        </div>
        <div className="flex items-center gap-4 bg-white border border-[#E5E7EB] p-3 rounded-lg shadow-sm">
          <div className="text-center border-r border-[#E5E7EB] pr-4">
            <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider block">Portfolio AUM</span>
            <span className="text-lg font-bold text-zinc-900 mt-0.5 block">
              MYR {appMode === 'gcfs' ? '21.18M' : '2.34B'}
            </span>
          </div>
          <div className="text-center">
            <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider block">Churn Warning</span>
            <span className="text-sm font-bold text-red-600 flex items-center gap-1 mt-0.5 justify-center">
        ...
        {tabs.map((tab, idx) => (
          <button
            ...
            className={`h-8 px-3 rounded-lg text-xs font-bold transition-all ${
              activeTab === tab 
                ? 'bg-card-bg text-page-body shadow-sm' 
                : 'bg-white hover:bg-zinc-50 text-zinc-500 hover:text-zinc-800 border border-[#E5E7EB]'
            }`}
          >
            {tab}
          </button>
      ...
      {activeTab === 'Day Plan' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[#fff] rounded-xl p-5 shadow-sm border border-[#E9EBEA] flex flex-col justify-between min-h-[300px]">
              <div>
                <div className="flex justify-between items-center pb-3 border-b border-zinc-100">
                  ...
                  <span className="text-[10px] font-bold text-[#58595A] bg-[#F4F4F6] px-2.5 py-1 rounded-full uppercase">
                    This Week
                  </span>
                </div>
                  ...
                  {customers.slice(0, 3).map((cust, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-xs font-bold text-[#292A2A] truncate">{cust.name}</h4>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[9px] text-brand font-extrabold uppercase bg-brand/10 px-2 py-0.5 rounded-full leading-none">
                            {cust.segment}
                          </span>
              ...
              <button 
                ...
                className="w-full text-center py-2 bg-white hover:bg-lime border border-[#DBDCDD] rounded-full text-[10px] font-extrabold text-[#292A2A] uppercase tracking-widest mt-4 transition-colors"
              >
                ...
                <div className="mt-4 space-y-3.5">
                  {customers.slice(0, 2).map((cust, idx) => (
                    <div 
                      key={idx}
                      onClick={() => viewCustomer(cust.id)}
                      className="p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-xl hover:border-blue-300 transition-all cursor-pointer"
                    >
                      ...
                      <h4 className="text-xs font-extrabold text-[#292A2A] mt-1.5 truncate">
                        {cust.name}
                      </h4>
                      <p className="text-[10px] text-[#58595A] font-semibold mt-1 leading-relaxed">
                        {appMode === 'gcfs' 
            ...
            <div className="bg-[#fff] rounded-xl p-5 shadow-sm border border-[#E9EBEA] flex flex-col justify-between min-h-[300px]">
              <div>
                ...
                <div className="mt-4 space-y-4">
                  {/* Meeting Progress */}
                  <div>
                    <div className="flex justify-between items-center text-[10px] font-extrabold text-[#58595A] uppercase tracking-wide">
                    ...
                    <div className="w-full h-2 bg-zinc-100 rounded-full mt-1.5 overflow-hidden">
                      <div className="w-1/2 h-full bg-lime" />
                    </div>
                  </div>

                  {/* Churn Warning Indicators */}
                  <div className="p-3 bg-[#FFE8F2] border border-[#F5C2D7] rounded-xl">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-extrabold text-error uppercase tracking-widest">
                        Churn Alert
                      </span>
                    ...
                    <p className="text-[10px] text-[#58595A] font-semibold mt-1.5 leading-relaxed">
                      {appMode === 'gcfs'
              ...
              <button 
                ...
                className="w-full text-center py-2 bg-[#FFE8F2] hover:bg-brand-300 border border-[#F5C2D7] rounded-full text-[10px] font-extrabold text-error uppercase tracking-widest mt-4 transition-colors"
              >
                Review Churn Portfolios
              </button>
            </div>
          </div>          {/* Row 2: Insights, New Leads/Cases, Appointments */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            
            {/* Column 1: Insight / Cross-sell Cards */}
            <div className="bg-[#fff] rounded-xl p-5 shadow-sm border border-[#E9EBEA] flex flex-col justify-between min-h-[350px]">
              <div>
                ...
                <div className="mt-4 space-y-3.5">
                  {appMode === 'gcfs' ? (
                    <>
                      ...
                      <div className="p-3.5 bg-[#FAF4F5] border border-[#F5C2D7] rounded-xl relative">
                      ...
                      <div className="p-3.5 bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 rounded-xl relative">
                        <h4 className="text-xs font-extrabold text-zinc-800 uppercase tracking-wide">
                    ...
                    <>
                      ...
                      <div className="p-3.5 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-xl relative">
                        <h4 className="text-xs font-extrabold text-zinc-800 uppercase tracking-wide">
                      ...
                      <div className="p-3.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 rounded-xl relative">
                        <h4 className="text-xs font-extrabold text-zinc-800 uppercase tracking-wide">
              ...
              <button onClick={() => setSummaryAction('insights')} className="w-full text-center py-2 bg-white hover:bg-lime border border-[#DBDCDD] rounded-full text-[10px] font-extrabold text-[#292A2A] uppercase tracking-widest mt-4 transition-colors">
                ...
                <div className="mt-4 space-y-3">
                  {opportunities.slice(0, 3).map((opp, idx) => (
                    <div 
                      ...
                      className="p-3 bg-white border border-[#E9EBEA] rounded-xl hover:border-brand transition-all cursor-pointer"
                    >
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] font-extrabold text-[#58595A] uppercase tracking-wide">
                          {opp.id}
                        </span>
                        <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full ${
                          opp.rating === 'Hot' ? 'bg-brand/10 text-brand' : 'bg-warning/10 text-warning'
                        }`}>
                          {opp.rating}
                        </span>
                      </div>
                      <h4 className="text-xs font-bold text-[#292A2A] truncate mt-1">{opp.name}</h4>
                      <div className="flex items-center justify-between text-[10px] text-[#58595A] font-semibold mt-1">
            ...
            <div className="bg-[#fff] rounded-xl p-5 shadow-sm border border-[#E9EBEA] flex flex-col justify-between min-h-[350px]">
              <div>
                <div className="flex justify-between items-center pb-3 border-b border-zinc-100">
                  ...
                  <span className="text-[10px] font-black text-[#292A2A] bg-lime px-2 py-0.5 rounded uppercase">
                    Today
                  </span>
                </div>
                ...
                <div className="mt-8 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-[10px] text-[#58595A] font-semibold mt-1 max-w-xs leading-relaxed">
                    No outstanding customer meetings or appointments logged for the remainder of today. Enjoy the clear slate!
                  </p>
                </div>
              </div>
              <button onClick={() => setSummaryAction('appointment')} className="w-full text-center py-2 bg-lime hover:bg-lime-hover rounded-full text-[10px] font-extrabold text-[#292A2A] uppercase tracking-widest mt-4 transition-all">
      ...
      {activeTab === 'QTD Engagements' && (
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-zinc-200 space-y-6 animate-fadeIn">
          <div className="border-b border-zinc-100 pb-3 flex justify-between items-center">
            ...
            <button className="h-8 px-4 rounded-xl bg-zinc-100 text-zinc-800 text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-opacity">
              + Log New Interaction
            </button>
          </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-zinc-200">
              ...
              <h3 className="text-xl font-black text-brand mt-2">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between items-center text-[10px] font-extrabold text-zinc-500 uppercase">
                  ...
                  <span className="text-brand">{appMode === 'gcfs' ? 'MYR 1.8M / 2.0M' : 'MYR 750M / 1.0B'}</span>
                </div>
                <div className="w-full h-2 bg-zinc-100 rounded-full mt-1.5 overflow-hidden">
                  <div className="h-full bg-brand" style={{ width: appMode === 'gcfs' ? '90%' : '75%' }} />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center text-[10px] font-extrabold text-zinc-500 uppercase">
                  ...
                  <span className="text-lime">{appMode === 'gcfs' ? 'MYR 2.05M / 2.50M' : 'MYR 1.45B / 1.50B'}</span>
                </div>
                <div className="w-full h-2 bg-zinc-100 rounded-full mt-1.5 overflow-hidden">
                  <div className="h-full bg-lime" style={{ width: appMode === 'gcfs' ? '82%' : '96%' }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-zinc-200 space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-semibold">
            ...
            <div className="p-4 border border-zinc-200 rounded-2xl bg-zinc-50">
              ...
              <span className="text-base font-black text-brand mt-1 block">
      ...
      {activeTab === 'Quick Actions' && (
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-zinc-200 space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <button 
              onClick={() => setActiveModule(appMode === 'gcfs' ? 'My Portfolio' : 'Accounts')}
              className="p-4 bg-gradient-to-br from-zinc-50 to-zinc-100 text-page-body rounded-2xl hover:opacity-95 transition-opacity text-left shadow border border-card-border"
            >
              <Briefcase className="w-5 h-5 text-lime mb-3" />
            ...
            >
              <TrendingUp className="w-5 h-5 text-brand mb-3" />
      ...
      >
        <div className="space-y-3 text-xs">
          {(summaryAction === 'appointment'
            ...
            <button key={item} onClick={() => setSummaryAction(null)} className="w-full p-3 rounded-lg border border-zinc-200 bg-zinc-50 text-left font-bold hover:border-brand">
              {item}
            </button>
          ))}
        </div>
